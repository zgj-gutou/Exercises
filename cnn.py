...
class CNNClassfier(object):
    def __init__(self,config):
        ...
        self.filter_sizes = config['filter_sizes']   # 过滤器大小
        self.num_filters = config['num_filters']  # 过滤器数目
    ...
    def build_graph(self):
        ...
        with tf.variable_scope("discriminator"):
            with tf.device("/cpu:0"),tf.name_scope("embedding"):
                self.W = tf.Variable(tf.random_uniform([self.vocab_size,self.embedding_size],-1,1),name = 'W')
                # 卷积层权重W，shape为[vocab_size, embedding_size], 符合[-1,1]一致分布
                self.embedding_chars = tf.nn.embedding_lookup(self.W, self.x)
                # embedding_lookup作用是从W权重矩阵中抽取对应x的权重矩阵，shape=batch_size * max_words * embedding_size
                self.embedding_chars_expanded = tf.expand_dims(self.embedding_chars,-1)
                # expand_dims作用是扩展一维数据，用于后面的卷积运算
            pooled_outputs = list()
            for filter_size,filter_num in zip(self.filter_sizes,self.num_filters):
                with tf.name_scope("cov2d-maxpool%s"% filter_size):
                    ...
                    # filter_size(做一次卷积运算窗口内的单词个数)、embedding_size为词向量维度、1为输入通道数、filter_num为卷积核个数
                    W = tf.Variable(tf.truncated_normal(filter_shape,stddev=0.1),name = "W")
                    b = tf.Variable(tf.constant(0.1,shape=[filter_num]),name="b")
                    conv = tf.nn.conv2d(self.embedding_chars_expanded,W,strides=[1,1,1,1],padding="VALID",name="conv")
                    h = tf.nn.relu(tf.nn.bias_add(conv,b),name = "relu")
                    pooled = tf.nn.max_pool(h,ksize=[1,self.max_len-filter_size+1,1,1],strides=[1,1,1,1],padding='VALID',name = 'pool')
                    # 池化后为1 * 1的元素，有多少个filter_num就有多少个1 * 1的元素
...
if __name__ == '__main__':
    print("main:")
    # load data 读入训练集和测试集
    x_train, y_train = load_data("./dbpedia_csv/train.csv", sample_ratio=1, one_hot=False)
    x_test, y_test = load_data("./dbpedia_csv/test.csv", one_hot=False)

    # data preprocessing
    x_train, x_test, vocab_size = \
        data_preprocessing_v2(x_train, x_test, max_len=120)
    # data_preprocessing_v2用于数据预处理
